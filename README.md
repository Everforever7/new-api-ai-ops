...
- 名称需要表达设备或运行位置、`酒馆`/`tt酒馆` 用途家族和具体用途；顺序、空格、大小写与分隔符不固定，AI 会识别 `ST`、`SillyTavern`、`TauriTavern` 等常见别名。
- 每批最多向 AI 提交 1000 个令牌的 ID、名称和最小用户元数据，不提交真实令牌密钥；AI 只输出异常项，响应异常时会自动缩小批次重试。
- 信息缺失或含义不明确的项目标记为人工复核；明确违规的封禁候选会交给第二轮 AI 独立判断。
- 多个异常令牌会合并为一个用户动作。只有两轮均确认、达到置信度阈值，且“封禁用户”确认策略设为自动时才会直接执行；默认仍进入人工审批。
