...
- 🔑 **用户令牌巡视** — AI 全量语义检查活动令牌，仅返回异常项，并按用户生成复核或封禁动作
...
| 📄 报告查看 | 查看最新生成的运维报告 |
| 📡 渠道快照 | 查看脱敏后的渠道信息 |
| 🔑 用户令牌 | AI 语义检查设备、酒馆用途与用途说明，并展示人工复核或二次确认结果 |
| 🤖 动作队列 | 查看 AI 建议动作，执行或拒绝待确认操作 |
| ⚙️ 执行设置 | 配置 AI 能力权限、确认策略、保护渠道规则 |

### 用户令牌 AI 巡视

- 默认每 60 分钟读取当前数据库中全部符合条件的活动令牌，而不是只检查最近有调用的用户。
- AI 根据令牌自身的 `token_group` 分流；空分组会按 `new-api` 语义继承用户分组。`default` 要求设备或运行位置、`酒馆`/`tt酒馆` 用途家族和具体用途；`代码` 只需写清 IDE/开发工具或开发环境及其作用，例如 `VSCode coding`、`Cursor 项目开发`、`PyCharm 脚本调试`。名称顺序、空格、大小写与分隔符不固定。
- 其他令牌分组不在巡视范围内，会在进入 AI 前跳过，也不会生成异常或审批动作；巡视界面会显示范围内异常令牌的分组。
- 每批最多向 AI 提交 100 个令牌的 ID、名称和最小用户元数据，不提交真实令牌密钥；AI 只输出异常项，响应异常时会按 50、25 条自动缩小批次重试。
- 信息缺失或含义不明确的项目标记为人工复核；明确违规的封禁候选会交给第二轮 AI 独立判断。
- 多个异常令牌会合并为一个用户动作。只有两轮均确认、达到置信度阈值，且“封禁用户”确认策略设为自动时才会直接执行；执行前会重新校验令牌 ID、名称和分组，默认仍进入人工审批。
...
| `NEWAPI_TOKEN_PAGE_SIZE` | 管理员令牌分页大小 | `100` |
...
- ✅ 支持的动作限定为测试渠道、低余额记录、创建渠道、修改渠道、禁用渠道、删除渠道、封禁用户
- ⚠️ 创建、修改、禁用、删除渠道均受设置页权限与确认策略约束
- 🛡️ 受保护的渠道 ID、分组、标签、名称、模型、类型会跳过 AI 修改
- 🛡️ 管理员、Root、SVIP 和配置的豁免用户分组不会生成封禁动作
- 📝 已执行、失败、拒绝的操作记录到 SQLite `data/ops.db`
