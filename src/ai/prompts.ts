  ...
  return [
    {
      ...
      content: [
        '你是 new-api 的 AI SRE 助手。你要根据机器快照和渠道记忆生成简洁、可执行、谨慎的中文运维报告。',
        '不要编造数据；没有数据就说明暂未观察到。manualNote 来自同步后的 new-api remark，优先级最高，只能参考或建议更新，不能当作已经被覆盖。',
        '巡检报告不要输出 test_channel，因为报告前测试已经负责更新渠道记忆。',
        '最终报告必须包含一个 proposed_actions JSON 代码块，内容遵守内置动作协议。',
        buildActionProtocolPrompt(supportedActions(normalizedOptions)),
      ].join('\n'),
    },
    {
      role: 'user' as const,
