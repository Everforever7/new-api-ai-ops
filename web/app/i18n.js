...
const dictionaries = {
  'zh-CN': {
    ...
    tokenInspection: {
      title: '用户令牌命名巡视',
      summary: '已检查 {tokens} 个活动令牌，涉及 {users} 个待复核用户',
      notRun: '尚未巡视',
      completedAt: '完成于 {value}',
      run: '立即巡视',
      running: '巡视中',
      findingsTitle: '命名策略发现',
      hint: '标准结构：设备名称/酒馆或tt酒馆/用途说明。违规用户会进入现有动作队列等待确认。',
    ...
    settings: {
      ...
      tokenInspection: {
        title: '用户令牌巡视',
        enabled: '启用定时令牌巡视',
        enabledHint: '按间隔读取所有活动令牌，只对不合规用户生成封禁审批。',
        aiReviewEnabled: '启用 AI 语义复核',
        aiReviewHint: 'AI 只复核结构正确但设备或用途含义模糊的名称，不生成执行动作。',
        policyTitle: '巡视策略',
        intervalMinutes: '巡视间隔分钟',
        graceHours: '新令牌宽限小时',
        maxActionsPerRun: '每轮最多用户动作',
        actionCooldownHours: '用户动作冷却小时',
        autoDisableConfidence: '自动封禁置信度',
        namingTitle: '命名规则',
        allowedClients: '允许的客户端名称',
        allowedClientsHint: '逗号分隔；标准结构固定为“设备名称/客户端/用途说明”。',
  ...
  'en-US': {
    ...
    tokenInspection: {
      title: 'User Token Naming Inspection',
      summary: 'Inspected {tokens} active tokens; {users} users need review',
      notRun: 'Not run yet',
      completedAt: 'Completed at {value}',
      run: 'Run inspection',
      running: 'Inspecting',
      findingsTitle: 'Naming policy findings',
      hint: 'Canonical format: device name/酒馆 or tt酒馆/purpose. Users with violations enter the existing action queue for confirmation.',
      notRunHint: 'Run an inspection to check active tokens in the current database.',
      empty: 'All active tokens currently match the naming policy.',
      user: 'User',
    ...
    settings: {
      ...
      tokenInspection: {
        ...
        enabledHint: 'Reads all active tokens on schedule and creates disable-user approvals only for non-compliant users.',
        aiReviewEnabled: 'Enable AI semantic review',
        aiReviewHint: 'AI only reviews structurally valid but ambiguous device or purpose text and never creates execution actions.',
        ...
        autoDisableConfidence: 'Auto-disable confidence',
        namingTitle: 'Naming rules',
        allowedClients: 'Allowed client names',
        allowedClientsHint: 'Comma separated; the canonical format is device/client/purpose.',
