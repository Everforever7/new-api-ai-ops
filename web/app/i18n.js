...
const dictionaries = {
  'zh-CN': {
    ...
    errors: {
      ...
      tokenInspectionRunFailed: '用户令牌巡视失败',
    },
    preferences: {
      language: '语言',
      theme: '主题',
    ...
    channels: {
      ...
      editNoteHint: '备注会同步写入 new-api remark，并提供给 AI；AI 只能参考或建议更新，不会静默覆盖。',
    ...
    tokenInspection: {
      title: 'AI 用户令牌语义巡视',
      summary: 'AI 已检查 {tokens} 个活动令牌，发现 {issues} 个异常，涉及 {users} 个用户',
      notRun: '尚未巡视',
      completedAt: '完成于 {value}',
      run: '立即巡视',
      running: '巡视中',
      findingsTitle: 'AI 异常判断',
      hint: 'default 分组按酒馆用途判断；代码分组只要求写清 IDE/开发工具和作用。待复核项进入审批队列；封禁项须经第二轮 AI 独立确认。',
      notRunHint: '点击“立即巡视”检查当前数据库中的活动令牌。',
      empty: 'AI 未发现需要处理的活动令牌。',
      user: '用户',
      tokenName: '令牌名称',
      tokenGroup: '令牌分组',
      severity: '处置级别',
    ...
    settings: {
      ...
      tokenInspection: {
        title: '用户令牌巡视',
        enabled: '启用定时令牌巡视',
        enabledHint: '默认每 60 分钟读取全部符合条件的活动令牌，AI 只返回异常项并按用户合并动作。',
        policyTitle: '巡视策略',
        intervalMinutes: '巡视间隔分钟',
        graceHours: '新令牌宽限小时',
        maxActionsPerRun: '每轮最多用户动作',
        actionCooldownHours: '用户动作冷却小时',
        autoDisableConfidence: '自动封禁最低置信度',
        namingTitle: 'AI 判定规则',
        allowedClients: '允许的客户端用途家族',
        allowedClientsHint: '仅用于 default 令牌分组，逗号分隔。代码分组固定按 IDE/开发工具 + 作用判断，不受此列表限制。',
        exemptUserGroups: '额外豁免用户分组',
        exemptUserGroupsHint: '管理员、Root 和 SVIP 已固定保护；此处可继续添加分组。',
      },
      report: {
        title: '巡检报告',
        scheduleTitle: '报告调度',
  ...
  'en-US': {
    ...
    errors: {
      ...
      tokenInspectionRunFailed: 'Failed to inspect user tokens',
    },
    preferences: {
      language: 'Language',
      theme: 'Theme',
    ...
    channels: {
      ...
      editNoteHint: 'Notes are synced to the new-api remark field and sent to AI. AI can reference or suggest changes, but cannot silently overwrite them.',
    ...
    tokenInspection: {
      title: 'AI User Token Semantic Inspection',
      summary: 'AI inspected {tokens} active tokens, found {issues} issues across {users} users',
      notRun: 'Not run yet',
      completedAt: 'Completed at {value}',
      run: 'Run inspection',
      running: 'Inspecting',
      findingsTitle: 'AI findings',
      hint: 'The default group follows the Tavern policy; the 代码 group only needs an IDE/development tool and its purpose. Review items enter approval; block items require a second AI decision.',
      notRunHint: 'Run an inspection to check active tokens in the current database.',
      empty: 'AI found no active tokens that require action.',
      user: 'User',
      ...
      verifications: {
        verified: 'Second pass confirmed',
    ...
    actions: {
      ...
      source: {
        ...
        token_inspection: 'Token inspection',
    ...
    settings: {
      ...
      tokenInspection: {
        title: 'User Token Inspection',
        enabled: 'Enable scheduled token inspection',
        enabledHint: 'By default, reviews every eligible active token every 60 minutes, returns only anomalies, and groups actions by user.',
        policyTitle: 'Inspection policy',
        intervalMinutes: 'Interval minutes',
        graceHours: 'New-token grace hours',
        maxActionsPerRun: 'Max user actions per run',
        actionCooldownHours: 'User action cooldown hours',
        autoDisableConfidence: 'Minimum auto-disable confidence',
        namingTitle: 'AI decision policy',
        allowedClients: 'Allowed client-use families',
        allowedClientsHint: 'Applies only to the default token group. The 代码 group uses the fixed IDE/development-tool + purpose policy and ignores this list.',
        exemptUserGroups: 'Additional exempt user groups',
        exemptUserGroupsHint: 'Administrators, Root, and SVIP are always protected; add other groups here.',
      },
      report: {
        title: 'Reports',
      ...
      permissions: {
        ...
        updateChannel: 'Enable/update channel',
